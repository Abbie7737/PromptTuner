...
async def run(
    prompt: str, task_description: Optional[str] = None, env_file: str = ".env"
) -> None:
    """
...
def main() -> int:
    ...
    parser.add_argument(
        "prompt", nargs="?", help="The prompt to optimize (or provide with -f/--file)"
    )
    parser.add_argument(
        "-f", "--file", help="Read prompt from a file instead of command line"
    )
    parser.add_argument(
        "-t", "--task", help="Description of what the prompt should achieve"
    )
    parser.add_argument(
        "-e", "--env", default=".env", help="Path to environment file (default: .env)"
    )
