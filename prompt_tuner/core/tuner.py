...
class PromptTuner:
    ...
    async def tune(
        self, original_prompt: str, task_description: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        print("This may take a few minutes for reasoning models...")

        initial_prompts = await self._generate_prompt_variations(
            original_prompt, task_description, self.num_initial_prompts
        )
        ...
        print(
            f"Running each prompt on the small model: {self.lmstudio.small_model.model}"
        )
        print("Then evaluating responses with the large model...")
        evaluations = await self._evaluate_prompts(
            initial_prompts, original_prompt, task_description
        )
        ...
        # Save raw results as JSON for later analysis first
        ...
        try:
            ...
            simple_report_path = os.path.join(
                self.results_dir, f"prompt_tuning_{run_id}_simple.md"
            )
...
"""
    ...
    async def _generate_prompt_variations(
        self, original_prompt: str, task_description: Optional[str], num_variations: int
    ) -> List[str]:
        """
        ...
        generator_prompt = self.prompt_loader.format(
            ...
            original_prompt=original_prompt,
            task_description=task_description
            or "No specific task description provided",
    ...
    async def _evaluate_prompts(
        self, prompts: List[str], original_prompt: str, task_description: Optional[str]
    ) -> List[Dict[str, Any]]:
        """
        ...
        for i, prompt in enumerate(prompts):
            ...
            evaluation = await self.lmstudio.run_on_large_model(
                messages=eval_messages, temperature=0.2
            )
            ...
            if score_matches:
                ...
                if score == 0:
                    print("WARNING: Could not extract a score from evaluation")
    ...
    async def _refine_prompt(
        ...
        if "```" in content:
            ...
            if len(parts) >= 3:  # At least one code block
                ...
                if refined_prompt.startswith("prompt") or refined_prompt.startswith(
                    "system"
                ):
